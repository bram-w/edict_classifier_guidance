...
def EDICT_editing(im_path,
    ...
    Args:
        ...
        guidance_scale: classifier-free guidance scale
            ...
            Higher can result more distortions
        run_baseline:
            VERY IMPORTANT
    ...
    Output:
        ...
        If run_baseline=False then they will be two nearly identical edited versions
            (usually identical, if there's drift other is often better,
            I forget which is cleaner for 50 steps, might depend on strength?
    """
...
##### BASELINE ALGO, ONLY USED NOW FOR SDEDIT ####3
...
def coupled_stablediffusion(prompt="",
                          ...
                          fixed_starting_latent=None,
                           beta_schedule='scaled_linear',
                            mix_weight=0.93,
                           collect_step2step_cosine_sims=False,
                           collect_pairwise_cosine_sims=False):
    #If seed is None, randomly select seed from 0 to 2^32-1
    if seed is None: seed = random.randrange(2**32 - 1)
    generator = torch.cuda.manual_seed(seed)
    ...
    def image_to_latent(im):
        if isinstance(im, torch.Tensor):
            ...
            return init_latent
    assert not use_lms, "Can't invert LMS the same"
    if run_baseline: leapfrog_steps=False
    if collect_step2step_cosine_sims:
        uc_c_grad_cosine_sim_tuples = [(None, None, None)]
        prev_uc_c_grad = None
    if collect_pairwise_cosine_sims:
    ...
    for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
        ...
        for latent_i in range(2): 
            ...
            if collect_step2step_cosine_sims:
                if prev_uc_c_grad is not None:
                    sim_tup = [F.cosine_similarity(t1.flatten(), t2.flatten(), dim=0).item()
                               for (t1, t2) in zip(prev_uc_c_grad, [noise_pred_uncond, noise_pred, grad])
                              ]
                    uc_c_grad_cosine_sim_tuples.append(sim_tup)
                prev_uc_c_grad = (noise_pred_uncond, noise_pred, grad)
        ...
        if collect_pairwise_cosine_sims:
            sim = F.cosine_similarity(*[l.flatten() for l in latent_pair], dim=0).item()
            pairwise_cosine_sims.append(sim)

    #scale and decode the image latents with vae
    if reverse or return_latents:
        results = [latent_pair]
        if collect_step2step_cosine_sims:
            results.append(uc_c_grad_cosine_sim_tuples)
        if collect_pairwise_cosine_sims:
            results.append(pairwise_cosine_sims)
